...
const PIPELINE_OPTIONS = [
  ...
  },
  {
    ...
    speed: "Slowest turnaround",
    continuity: "Continuity cues only",
    evidence: "Densest evidence set",
    note: "Heavier evidence-building pass for analyst-led review, not a guaranteed better operational mode.",
    useCase: "Best for researcher inspection, denser saved frames, and slower side-by-side review.",
    emphasis: "Analyst mode",
  },
];
// 1. Add your new key to the visible list
const VISIBLE_MODEL_KEYS = ["yolo26_v3", "labeling_data_v2_s", "roboflow_v1_fresh2"];
...
const MODEL_META = {
  yolo26_v3: {
    label: "YOLO26 Model",
    badge: "New",
    note: "Newly trained model for drone-based elephant recognition.",
  },
  labeling_data_v2_s: {
    label: "Research Model v3_2s",
    badge: "Online",
    note: "Current stable research model.",
  },
  roboflow_v1_fresh2: {
    label: "Roboflow Online v1",
    badge: "External",
    note: "Pre-trained model from Roboflow dataset.",
  },
};
...
const PROCESSING_STAGES = [
  ...
  { label: "Linking evidence", note: "Estimating continuity cues across sampled detections.", accent: "Continuity cues" },
...
function appStateModeFromMetrics(metrics) {
  ...
  return mode === "quality" ? "Research Review" : mode === "fast_trend" || mode === "fast" ? "Fast Trend" : "-";
}
...
const availableModelEntries = useMemo(() => {
  ...
  // This is the fallback list you are currently seeing
    return [
      ["yolo26_v3", ""],
      ["labeling_data_v2_s", ""],
      ["roboflow_v1_fresh2", ""],
...
function HistoryPage({ jobs, loading, error, selectedJobId, onSelectJobId, onRefresh, navigate }) {
  ...
  const filteredJobs = useMemo(() => {
    ...
    return jobs.filter((job) => {
      ...
      const matchesQuery = !normalized || filename.toLowerCase().includes(normalized);
      const isImage = /\.(jpg|jpeg|png)$/i.test(filename);
      const matchesMedia =
        mediaFilter === "all" || (mediaFilter === "image" ? isImage : !isImage);
      return matchesQuery && matchesMedia;
    });
  }, [jobs, query, mediaFilter]);
  ...
  return (
    <>
      ...
      <section className="history-toolbar">
        <div className="history-toolbar-bar">
          ...
          <div className="history-filter-group" role="tablist" aria-label="Media type filter">
            <button
              type="button"
              className={clsx("history-filter-chip", mediaFilter === "all" && "history-filter-chip-active")}
            ...
            <button
              type="button"
              className={clsx("history-filter-chip", mediaFilter === "image" && "history-filter-chip-active")}
            ...
            <button
              type="button"
              className={clsx("history-filter-chip", mediaFilter === "video" && "history-filter-chip-active")}
              onClick={() => setMediaFilter("video")}
            >
              Videos
            </button>
          </div>
          <div className="page-actions history-toolbar-actions">
        ...
        <div className="history-toolbar-meta">
          <span>{filteredJobs.length} visible runs</span>
          <span>{mediaFilter === "all" ? "All media" : mediaFilter === "image" ? "Images only" : "Videos only"}</span>
        </div>
      </section>
      ...
      {!filteredJobs.length && !loading ? (
        <EmptyState title="No matching analyses" body="Try a different filename, media filter, or date search." />
      ) : null}
